...
import {
  ...
  Settings, Sprout, DoorOpen, Coins, User, Menu,
} from "lucide-react";
...
const statusColor = (s) => ({ Healthy: C.safe, Safe: C.safe, Warning: C.warn, Critical: C.crit, Offline: C.offline }[s] || C.inkDim);
const statusSoft  = (s) => ({ Healthy: C.safeSoft, Safe: C.safeSoft, Warning: C.warnSoft, Critical: C.critSoft, Offline: C.offlineSoft }[s] || C.panelAlt);
...
const ICONS = {
  ...
  coins: Coins, user: User, menu: Menu,
};
...
const BOUNDS = { latMin: 22.3, latMax: 29.0, lngMin: 89.0, lngMax: 97.0 };
...
function genUnits(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out.push({
      ...
      capacityL: pick([3, 10, 20, 20, 30, 50]),
...
const UNITS = genUnits(18);

function genFreshness() {
  return UNITS.map((u) => {
...
const FRESHNESS = genFreshness();

function genAlerts() {
  const items = [];
  UNITS.filter((u) => u.status === "Critical").forEach((u) => items.push({ id: `AL-${u.id}-T`, sev: "Critical", cat: "Temperature", channel: pick(["SMS", "IVR Call"]), title: `Temperature excursion at ${u.village}`, sub: `${u.temp}°C · ${u.produce} · battery ${u.battery}%`, ago: rint(2, 90) }));
  UNITS.filter((u) => u.battery < 20).forEach((u) => items.push({ id: `AL-${u.id}-B`, sev: "Critical", cat: "Power", channel: "SMS", title: `Low battery — ${u.village}`, sub: `${u.battery}% remaining · mode: ${u.mode}`, ago: rint(1, 60) }));
  UNITS.filter((u) => u.status === "Warning").forEach((u) => items.push({ id: `AL-${u.id}-W`, sev: "Warning", cat: "Storage Condition", channel: pick(["App", "SMS"]), title: `Storage drifting out of range — ${u.village}`, sub: `${u.temp}°C / ${u.humidity}% RH`, ago: rint(10, 300) }));
  FRESHNESS.filter((f) => f.spoilageRisk > 55).forEach((f) => items.push({ id: `AL-${f.unitId}-F`, sev: "Warning", cat: "Spoilage Risk", channel: "IVR Call", title: `${f.produce} at ${f.village} — high spoilage risk`, sub: `${f.spoilageRisk}% risk · ${f.dispatchWindow}`, ago: rint(15, 400) }));
  UNITS.filter((u) => u.lastPowerFailureH < 12).forEach((u) => items.push({ id: `AL-${u.id}-P`, sev: "Warning", cat: "Power Failure", channel: "SMS", title: `Grid power failure detected — ${u.village}`, sub: `Running on ${u.mode} · ${u.lastPowerFailureH}h ago`, ago: u.lastPowerFailureH * 60 }));
...
const HARDWARE = [
  ...
  { id: "compressor", name: "DC Compressor Cooling Unit", cat: "Cooling", x: 50, y: 46, color: C.cold, spec: "12V/24V variable-speed DC compressor (or TEC1-12706 Peltier module for the 3L prototype), rated for low-power solar-DC operation.", role: "Core refrigeration element — draws heat from the insulated chamber to hold target temperature." },
  ...
  { id: "sensor", name: "DHT22 Temp & Humidity Sensor", cat: "Sensing", x: 74, y: 52, color: C.veg, spec: "Digital temperature (±0.5°C) and humidity (±2% RH) sensor mounted inside the chamber.", role: "Feeds live readings to the microcontroller for closed-loop cooling control and alerting." },
  { id: "mcu", name: "ESP32 Microcontroller", cat: "Control", x: 74, y: 30, color: C.coop, spec: "ESP32 dev board — WiFi/BLE + low-power modes, runs the control firmware and edge logic.", role: "Reads sensors, drives the compressor relay, triggers alerts, and syncs unit status." },
  { id: "gsm", name: "GSM/SIM800L SMS Module", cat: "Connectivity", x: 88, y: 20, color: C.coop, spec: "2G GSM module for SMS/IVR alerts in areas with no smartphone data connectivity.", role: "Sends farmer-facing SMS/IVR alerts for power failure, temperature excursion & dispatch reminders." },
  { id: "display", name: "OLED Status Display", cat: "Interface", x: 26, y: 38, color: C.earth, spec: '0.96" OLED display showing live temperature, humidity, battery % and mode.', role: "On-unit digital display so cooperative operators can check status without a phone." },
  { id: "buzzer", name: "Buzzer + LED Alert Indicator", cat: "Interface", x: 16, y: 56, color: C.warn, spec: "Piezo buzzer + tri-colour LED for audible/visual local alerts.", role: "Immediate on-site warning for door-open, over-temperature or low-battery events." },
  { id: "relay", name: "DC-DC Converter + Relay Module", cat: "Power", x: 62, y: 58, color: C.battery, spec: "Buck converter regulating battery voltage to logic/compressor rails, with relay-driven load switching.", role: "Distributes and switches power intelligently — enables the Eco / Load-Shedding mode." },
  { id: "doorsensor", name: "Door / Lid Open Sensor", cat: "Sensing", x: 32, y: 78, color: C.veg, spec: "Magnetic reed switch on the chamber lid.", role: "Detects door-open events to log cold-chain breaks and cue the buzzer." },
];
...
function Ribbon({ history, height = 22, color = C.cold }) {
  const vals = history.map((h) => h.v);
  const min = Math.min(...vals, 0), max = Math.max(...vals, 1), range = max - min || 1;
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 1, width: "100%", overflow: "hidden", height }}>
      {history.map((h, i) => {
        const pct = (h.v - min) / range;
        return <div key={i} style={{ flex: 1, opacity: 0.85, borderRadius: 1, height: `${12 + pct * 88}%`, background: color }} title={String(h.v)} />;
      })}
    </div>
  );
}
...
/* ===================== NER deployment map — real OpenStreetMap tiles ===================== */
const TILE = 256;
...
function NerMap({ units, onSelect, height = 420, selectedId }) {
  ...
  const [view, setView] = useState(null); // { center:{lat,lng}, zoom }
...
/* ===================== Overview section ===================== */
function Overview({ search, setSection, openUnit }) {
    ...
    <div>
      <div style={{ marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12.5, color: C.inkFaint, marginTop: 4, fontWeight: 600 }}>Sense → Store → Alert → Dispatch — off-grid, on solar, across NER</div>
        </div>
...
/* ===================== Units section ===================== */
function UnitsSection({ search, openUnit }) {
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Panel title="Smart Load-Shedding" iconName="zap" iconColor={C.veg}>
            <div style={{ fontSize: 12.5, color: C.inkDim, lineHeight: 1.6 }}>
              When battery SOC drops below <b style={{ color: C.ink }}>25%</b>, the controller automatically enters <b style={{ color: C.veg }}>Eco Mode</b> — reducing compressor duty cycle and non-essential display brightness to protect the cold chain until solar recharge resumes at sunrise.
            </div>
          </Panel>
        </div>
      </div>
...
/* ===================== Freshness section ===================== */
function FreshnessSection({ search, openUnit }) {
...
function TwinSection() {
  ...
  return (
    <div>
      <PageHead title="Digital Twin — 3L Prototype Unit" subtitle="Interactive hardware layout · click a component to see its role (Fusion 360 build reference)" />
      <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 14 }}>
        <Panel title="Component Layout" iconName="cpu" iconColor={C.coop}>
          ...
          <div style={{ marginTop: 10, fontSize: 11, color: C.inkFaint }}>Dashed outline = insulated 3L chamber footprint. Coloured dots = mounted components — tap any dot for its spec sheet.</div>
        </Panel>
        <Panel title={selected.name} iconName="settings" iconColor={selected.color} right={<CatBadge cat={selected.cat} color={selected.color} compact />}>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 11, color: C.inkFaint, textTransform: "uppercase", fontWeight: 800, marginBottom: 5 }}>Role in the system</div>
...
function AlertsSection({ search }) {
  ...
  return (
    <div>
      <PageHead title="Alerts" subtitle="Delivered via SMS, IVR call, and the field display — built for zero-smartphone reach" />
      ...
      <Panel title="Leaderboard" iconName="trophy" iconColor={C.coop}>
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          <div style={{ display: "grid", gridTemplateColumns: "34px 1.3fr 90px 100px 110px 90px", gap: 8, padding: "4px 8px", fontSize: 10.5, color: C.inkFaint, textTransform: "uppercase", fontWeight: 800 }}>
            <div>#</div><div>Co-op / Village</div><div>Spoilage ↓</div><div>Produce saved</div><div>Income ↑</div><div>Diesel saved</div>
          </div>
          {COOP.map((c, i) => (
...
function MapSection({ search, openUnit }) {
  const [filter, setFilter] = useState("All");
...
function UnitModal({ unitId, onClose }) {
  ...
  const timeline = [
    { l: "Solar panel began charging at sunrise", t: "6h ago" },
    { l: `Sensor logged ${unit.temp}°C / ${unit.humidity}% RH`, t: "4m ago" },
    { l: fresh ? `Dispatch advisory generated: ${fresh.dispatchWindow}` : "No dispatch advisory pending", t: "6m ago" },
    { l: "Status synced to cooperative dashboard", t: "2m ago" },
  ];
  return (
      ...
      <div style={{ width: 800, maxWidth: "100%", maxHeight: "88vh", overflowY: "auto", background: C.panel, border: `1px solid ${C.line2}`, borderRadius: 19, boxShadow: "0 30px 60px -20px rgba(30,27,22,.4)" }}>
        ...
        <div style={{ padding: 22, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <AreaTrend history={unit.tempHistory} color={C.cold} height={160} refLines={[{ y: 8, color: C.warn, label: "upper safe" }, { y: 2, color: C.safe, label: "lower safe" }]} />
          </div>
          {fresh && (
...
export default function App() {
  ...
  const sections = {
    overview: <Overview search={search} setSection={setSection} openUnit={openUnit} />,
    units: <UnitsSection search={search} openUnit={openUnit} />,
    energy: <EnergySection search={search} />,
    freshness: <FreshnessSection search={search} openUnit={openUnit} />,
    twin: <TwinSection />,
    alerts: <AlertsSection search={search} />,
    coop: <CoopSection />,
    analytics: <AnalyticsSection />,
    map: <MapSection search={search} openUnit={openUnit} />,
  };

  return (
    <div style={{ height: "100vh", width: "100%", background: C.bg, color: C.ink, fontFamily: "'Inter',-apple-system,sans-serif", overflow: "hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        .cr-shell{display:flex;height:100vh;position:relative;}
        .cr-main{flex:1;min-width:0;display:flex;flex-direction:column;}
        .cr-content{padding:24px 26px;flex:1;overflow-y:auto;}
        @media (min-width: 921px){ .cr-sidebar{ position:relative !important; left:0 !important; } }
      ...
      <div className="cr-shell">
        <Sidebar section={section} setSection={setSection} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
      ...
      <UnitModal unitId={modalUnitId} onClose={() => setModalUnitId(null)} />
    </div>
  );
}
